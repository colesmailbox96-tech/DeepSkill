...
import { CURRENCY_SYMBOL, CURRENCY_PLURAL } from '../../engine/economy'
...
export function PlayerStrip() {
  ...
  return (
    <div className="hud-player-strip" role="status" aria-label="Player status">
      ...
      <div className="hud-coins" aria-label={`${coins} ${CURRENCY_PLURAL}`}>
        <span className="hud-coins__icon">{CURRENCY_SYMBOL}</span>
        <span className="hud-coins__value">{coins}</span>
        <span className="hud-coins__label">{CURRENCY_PLURAL}</span>
      </div>
    </div>
  )
}
